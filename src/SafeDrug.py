...
def ablate_feature(self, feature):
    ...
    if feature.lower() in lowercase_columns:
        print("Ablating feature {}".format(feature))
...
def main():
    ...
    ablation_study = AblationStudy('records_final',training_dataset_version=1, label_name='ICD-9', dim=args.dim)
    ablation_study.model.set_base_model_generator(SafeDrugModel)
    i=0
    if(i==0):
        ablation_study.model.layers.include(2, 4)
    else:
        ablation_study.model.features.exclude("Prescriptions")

    result = experiment.lagom(experiment_type='ablation', ablation_study=ablation_study, ablator='loco_pytorch', name='SafeDrug', direction="min")
    ...
    plt.plot(epochs, jacc, color="blue", label="Jaccard")
    plt.plot(epochs, PRAU, color="red", label="PRAUC Score")
    plt.plot(epochs, F1S, color="green", label="F1 Score")
    plt.plot(epochs, DDI, color="orange", label="DDI Rate")
    plt.legend()
    plt.title('Metrics for '+model_name)
    plt.xlabel('Epochs')
    plt.ylabel('Metrics')
    plt.savefig('save_'+model_name+'.png',dpi=100)
